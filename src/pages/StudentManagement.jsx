...
import { Search, Filter, Eye, Edit, UserPlus, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function StudentManagement({ user, onLogout }) {
  ...
  let data = allStudents.filter(s => {
    ...
    const riskMatch = filter === 'All Students' || s.risk === filter;
            ...
            <tbody>
              {data.map((s) => (
                <tr key={s.id}>
                  ...
                  <td><GradeBadge grade={s.predictedGrade} /></td>
                  <td>
